...
def save_file_random(image: np.ndarray, labels: list, bboxes: list, save_dir: str) -> None:
        ...
        save_image(image, os.path.join(save_dir, "images"), file_name)
        save_label(labels, bboxes, os.path.join(save_dir, "labels"), text_name)


def save_image_random(image: np.ndarray, save_dir: str) -> None:
    name = uuid.uuid1()
    file_name = f"{name}.jpg"
    save_image(image, save_dir, file_name)
